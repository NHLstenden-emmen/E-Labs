...
if(isset($_GET['labjournal_id'])){
    $labusers = $db->GetAllLabUsers($_GET['labjournal_id']);
...
if(isset($_GET['preperation_id'])){
    $prepusers = $db->GetAllPreparationUsers($_GET['preperation_id']);
    while($users = $prepusers->fetch_array(MYSQLI_ASSOC)){
        $prepuser = $users['user_id'];
        if($user == $prepuser){  
            $preperation_id = $_GET['preperation_id'];
            $output = $db->selectcontentpreparation($preperation_id);
